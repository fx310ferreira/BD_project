...
#? Is having a role a problem?
#? admin should pass the token???
#! add procdure to add_song
...
def add_user():
    ...
    if 'zip_code' not in payload:
        ...
        return flask.jsonify(response), 400 # TODO add zip-code validation
    ...
    try:
        ...
        if(role == 'ARTIST'):
            ...
            values = (payload['artistic_name'], payload['admin_id'], payload['publisher_id'], login_id)
            cur.execute(statement, values)
        else:
...
def find_user(id, role, artist_id):
    ...
    try:
        ...
        response = {"artistic_name": response[0][0], "publisher_id": response[0][1], "songs": response[0][2], "albums": response[0][3]}
        response = flask.jsonify({'status': 200, 'results': f'{response}'}), 200
...
@app.route('/comments/subscription/', methods=['POST'])
@token_required
def add_comment(id, role):
    ...
    if 'period' not in payload and payload['period'].upper() not in ['MONTH', 'QUARTER', 'YEAR']:
        return flask.jsonify({'status': 400, 'errors': 'Missing period'}), 400
    if 'cards' not in payload and len(payload['cards']) < 1:
        return flask.jsonify({'status': 400, 'errors': 'Missing cards'}), 400

    try:
        statement = f"SELECT * FROM plan WHERE period = '{payload['period'].upper()}' AND active = TRUE"
        ...
        statement = 'INSERT INTO subscription (movement_date, )'
        values = [payload['comment'], song_id, id, parent_comment_id]
        cur.execute(statement, values)
        conn.commit()
        response = cur.fetchone()[0]
