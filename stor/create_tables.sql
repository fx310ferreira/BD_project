...
CREATE OR REPLACE FUNCTION update_top_10_playlist() RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    DELETE FROM top_10_order
    WHERE consumer_id = NEW.consumer_id;
	
    INSERT INTO top_10_order (song_order, consumer_id, song_ismn)
    SELECT RANK() OVER (ORDER BY COUNT(*) DESC), consumer_id, song_ismn
    FROM activity
    WHERE consumer_id = NEW.consumer_id AND NOW() - NEW.listen_date < (INTERVAL '1 month')
    GROUP BY song_ismn, consumer_id
    LIMIT 10;
	RETURN NULL;
END;
$$;
...
INSERT INTO login (id,username,email,password, user_privilege) VALUES 
(1, 'admin1','admin1@songhub.com','admin1', 'ADMIN'),
(2, 'admin2','admin2@songhub.com','admin2', 'ADMIN');

INSERT INTO administrator (login_id) VALUES 
(1),
(2);